...
export const Pokemon = () => {
    const [pokemonList, setPokemonList] = useState({id:0, name:"", image:"", gif:""});
    const [hasWon, setHasWon] = useState(false);

    const pokemonData = (num:number) => {
        fetch(`https://pokeapi.co/api/v2/pokemon/${num}/`)
        .then(res => res.json())
        .then(data => setPokemonList({
            id: data.id,
            name: data.name,
    ...
    return(
        <div className="h-screen bg-hero-pattern bg-no-repeat bg-cover flex items-center justify-center bg-center p-3">
            <div className="bg-black/80 rounded-xl min-h-[548px] max-w-2xl max-h-2xl w-full h-fit flex items-center justify-center flex-col p-5">
                 <img className={`max-h-[350px] max-w-[350px] w-full h-full pixelated ${hasWon ? "p-10" : "brightness-0 invert"}`}
                     src={hasWon ? pokemonList.gif : pokemonList.image} alt="pokemon" />
                 {hasWon ? (
                    <button onClick={playAgain} className="nes-btn is-success" autoFocus>Play again</button>
                 ) : (
                    <form onSubmit={handleSubmit} className="w-fit flex flex-col justify-center">
                        <input type="text" id="name_field" className="nes-input outline-none" name="pokemon" autoFocus autoComplete="off" />
